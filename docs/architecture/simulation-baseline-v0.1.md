...
Phase C  交互（显式延后）
          回放暂停 → 注入修正 → 断点续跑。
          注入只能以 ScenarioStimulus / capability input /
          有界 lifecycle command 形式进入，
          不得直写 canonical state（承产品基线 §24.2 / §24.8——A10 修订）。
          触发条件：≥3 次真实调试中回放-only 不足，或 owner 显式请求
          （A14 建议量化，待确认）。
...
| A14 | 自我反驳 | 分界保持；Phase C 触发条件量化（待确认） |
