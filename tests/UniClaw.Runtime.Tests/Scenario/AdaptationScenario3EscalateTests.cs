...
/// <summary>
/// Adaptation SC-3: authority boundary exceeded → Decision Escalate → Escalate adaptation.
/// The Fake world exposes an ancestry cycle; the DFS fails closed on Open-world identity
/// safety (authority boundary); the reconciler classifies Escalate; the ledger applies an
/// Escalate adaptation — the hypothesis is marked Revised with an escalation reason,
/// recording the inability. NO automatic recovery, retry, or action dispatch: the run
/// ends in the DFS's own Failed state and the adaptation only records.
/// </summary>
public sealed class AdaptationScenario3EscalateTests
{
    ...
    [Fact]
    public async Task AuthorityBoundaryExceeded_ProducesEscalateRecord_WithNoAutomaticRecovery()
    {
        ...
        var ledger = new ExecutionHypothesisLedger(resolved, "adapt-s3-run");
        var state = await DirectiveExecution.RunDirectiveAsync(
            agent, resolved, "adapt-s3-run", CancellationToken.None, ledger);
        ...
        // The reconciler classifies Escalate, and the adapter maps it to an Escalate
        // adaptation that RECORDS the inability (Revised + escalation reason).
        Assert.Equal(RuntimeDecisionState.Escalate, ledger.LatestDecision!.State);
        ...
        Assert.Equal("adapt-s3-run", adaptation.RunId);
        ...
        // NO automatic recovery / retry: the run is exactly the DFS's own fail-closed
        // result (agent.State == state == Failed) and only the bounded cycle dispatch
        // (one Tap) occurred — no recovery actions were added by the adaptation.
        ...
        Assert.DoesNotContain(environment.ActionHistory, action => action is DeviceAction.SystemBack);
    }
