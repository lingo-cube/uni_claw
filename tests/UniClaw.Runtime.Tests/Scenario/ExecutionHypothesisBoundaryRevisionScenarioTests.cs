...
/// <summary>
/// SC: run-local execution hypothesis boundary revision, Fake World. A bounded
/// exploration directive decomposes into an initial hypothesis; the DFS engine then
/// encounters an external boundary (trace records EXTERNAL_BOUNDARY_OBSERVED), the
/// hypothesis is revised with the boundary reason, the DFS handles the boundary
/// (returns to parent) and continues, and the run completes. The history shows the
/// revision with the boundary reason, and execution authority is unchanged — the
/// RunState is the Agent's DFS result, never the hypothesis's.
/// </summary>
public sealed class ExecutionHypothesisBoundaryRevisionScenarioTests
{
    ...
    [Fact]
    public async Task BoundaryRevisionScenario_RevisesHypothesis_WithBoundaryReason_AndKeepsAgentAuthority()
    {
        ...
        // Hypothetical objective must be generic (no scenario strings).
        var ledger = new ExecutionHypothesisLedger(resolved, "hyp-boundary-run");

        var state = await DirectiveExecution.RunDirectiveAsync(
            agent, resolved, "hyp-boundary-run", CancellationToken.None, ledger);

        // The DFS truly encountered + handled the boundary (auditable in the trace).
        ...
        // ledger never decides the outcome (it records, it does not determine it).
        Assert.Equal(agent.State, state);

        // The hypothesis was revised with the boundary reason: the history contains a
        // Revised hypothesis whose RevisionReason records the boundary, and a Replaced
        // hypothesis superseded by a "continue siblings" Created hypothesis.
        ...
        Assert.Contains(ledger.History, h => h.Status == ExecutionHypothesisStatus.Replaced);
        Assert.Contains(ledger.History,
            h => h.Status == ExecutionHypothesisStatus.Created
                && h.Objective.Contains("Continue remaining siblings", StringComparison.Ordinal));

        // The hypothesis ran under the initial generic objective derived from the
        // directive's declared scope (no scenario strings).
        Assert.Equal("Explore declared scope within bounded depth", ledger.History[0].Objective);
    }
}
