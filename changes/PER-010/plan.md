...
4. **Grill/disposition**：按 12 个攻击问题逐项给出 expected/actual；必要修改只限本文档；focused re-grill 后把 design_status 置为 FROZEN。
...
- A2 stable contract 明确 CaptureMetadata、Windows、Nodes、Capabilities、Observed/Unknown/Unsupported、Checked 三态和 Coverage。
...
actual: PASS，inventory 与 stable contract 已建立；focused re-grill 通过；待最终 exact-path audit。
