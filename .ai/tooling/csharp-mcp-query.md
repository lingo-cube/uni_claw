...
> 本文件是跨 AI Coder 的「C# 代码查询 MCP 优先」指南。
> - `AGENTS.md` 与通用工作流按需引用本文件
> - Host adapter 不复制本文件内容
> - 本指南不产生架构权威，也不保证某个 Host 已安装对应 MCP
...
| `cwm-roslyn-navigator` | `cwm-roslyn-navigator --solution <sln>` | **备用 Roslyn 导航服务**：`find_symbol`, `find_references`, `get_type_hierarchy`, 死代码检测, 反模式检测。主语义服务未初始化时可用它兜底 |
