# UniClaw Profile-based UniFlow Coding Workflow
...
> Host adapters: `.codex/` + `.dsh/`; portable discovery: `.agents/skills/`
...
| `engineering-governance` | `.ai/`、`.agents/`、`.codex/`、`.dsh/`、`openspec/`、`docs/`、`scripts/`、`tools/` | Coding Harness、OpenSpec/知识治理与一致性工具 |
...
```text
1. Tool Only 获取确定性事实
2. 在语义归因、架构判断或代码深挖前完成 Reality Preflight
3. 由最近 falsifier / First Divergence 识别一个主要 ModuleProfile
4. 选择 ExecutionProfile
5. 按任务类型选择最小 required_skills 集合
6. 冻结本次 change principles、contract 与 acceptance
7. 生成一个 WorkItem，填写精炼中文 semantic_brief，并指定唯一 worker_owner
8. 校验 WorkItem 与 required Skill 唯一解析结果
9. 单播给一个 Worker；禁止同一 WorkItem fanout
10. 接收精简 WorkResult，独立核对仓库与验证证据
11. Leader 接受后才应用 ModuleContext Delta
12. Leader 完成最终验证与用户输出
```

Reality Preflight 是短暂的工作视图，不是额外文档或新权威。Leader 用最少文字明确：

```text
用户可见目标 / 当前可观察状态 / 人类最短可行路径 / 预期可见变化 /
已观察偏差或未知项 / 最近 falsifier 或 First Divergence / 最小 owning seam
```

它用于抵抗两种过早收敛：仅凭症状给出语义归因，以及在尚未找到 First Divergence
时沿漫长调用链深挖。界面证据不可得时明确写“未知”，改用最近的仓库、结果或 trace
证据，不得编造界面状态。找到最小 owning seam 后停止横向扩张，再做 Profile、Skill
和 Owner 路由。该工作视图不得成为 Fact、Contract、Runtime belief、固定点击脚本或
修改授权。
...
Worker 不接收完整主线程。Leader 传递 WorkItem 后，Worker运行；WorkItem 已落盘时
优先使用第一种，内联 WorkItem 使用第二种并按原顺序重复 `--required-skill`：

```bash
python3 tools/agent_profile_validator.py context --work-item <work-item.json>
...
+ 目标路径生效的 AGENTS.md
+ WorkItem.required_skills 唯一解析得到的 canonical SKILL.md 路径
+ required_skill_context 中按既定顺序携带的 Skill 完整正文与 fail-closed 加载指令
...
`required_skills` 是向后兼容的有序 Skill 名称数组：旧 WorkItem 缺省为空；所有新建
WorkItem 显式输出该字段。Validator 只从 `.ai/skills` 解析正文；缺失、重复请求、
非法名称、不可读或 frontmatter 不匹配时，在 Worker 动作前 fail-closed。
Skill 只提供执行方法，`Authority: NONE`，不进入正式约束优先级，也不得扩大 scope、
权限、ownership、lifecycle、contract 或 acceptance。
...
## 6.1 Worker 疑难上报准则（所有通用 AI Coder Host 共用）

Worker 遇到以下情况**必须主动上报 Leader 寻求裁决，不得自行闷头解决**：

- ① 规约、契约或 invariant 有歧义或冲突；
- ② 实现路径有多种合理选择且影响行为语义；
- ③ 测试失败原因不明、反复修复无效、或疑似环境/硬件问题；
- ④ scope 之外发现需要改动的文件；
- ⑤ 任务假设与仓库实际不符。

上报方式：Worker 在 WorkResult 中设 `status` 为对应的 `BLOCKED_*`，并在 `unresolved`
列出具体问题、已尝试的方案和证据。**不自行扩大 scope、不猜测语义、不放宽 fail-closed。**

Leader 收到上报后裁决：

- A. 自己解决（架构/语义/invariant/authority 类，Leader 直接处理）；
- B. 给出方案与思路，生成新 WorkItem 派发给原 Worker 或对应模块 Worker 执行；
- C. 冻结追加决策后重派（Leader 补充 `change_principles` / `contract_refs` 再派发）。

此准则为**通用行为指导**，所有 Host 的 module-worker 触发
UniFlow 时均按此执行。DSH 侧由共享 manifest 的 `required_skill_context.directive`
与完整 Skill documents 进入 `worker_payload`；延迟派发记录必须原样保存该 payload，
见 §10 DSH 边界。
...
| WorkItem 类型 | required_skills |
|---|---|
| 普通实现、确定性机械任务 | `[]` 或任务明确需要的单一方法 Skill |
| Bug / 失败调查 | `evidence-driven-debugging` |
| Runtime / FSM / Traversal / Recovery / Async / 真机 / flaky / nondeterministic | 先 `evidence-driven-debugging`，再 `runtime-behavior-debugging` |

Leader 负责基于任务事实做上述分类；不得把 Skill 名埋进 `semantic_brief` 代替结构化
字段。多个 Skill 是顺序执行的方法组合，不是多 Worker fanout。
...
DSH 在收到 `UniFlow` 后按需消费 `.ai/profiles/*.json`、两个 schema、WorkItem/WorkResult 和路由规则。DSH adapter 必须在 Host spawn 前校验 `worker_payload` 中的 Skill name/path/order、完整正文摘要和 fail-closed 指令；延迟到会话侧 spawn 时，dispatch record 必须携带完整 `worker_payload`，不得要求会话侧重新猜测或解析 Skill。若当前 DSH adapter 不具备独立委派能力，则由主执行者按同一 WorkItem 边界内联执行并显式记录能力限制，不得改变 Profile 语义。以下职责仍留给未来 DSH：持久化线程池、跨会话缓存、队列/调度、并发冲突锁、自动验收状态机与 provider runtime。当前实现不创建独立 Agent Runtime、不实现通用 fanout，也不自动修改权威 ModuleContext。
