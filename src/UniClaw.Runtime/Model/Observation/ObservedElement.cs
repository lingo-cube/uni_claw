...
{
    private string? _stabilizerHint;

    /// <summary>
    /// 可选感知稳定器 hint（如 perception row_id）。它是瞬态 correlation
    /// evidence，不是元素身份、LogicalItem identity 或 action authority。
    /// </summary>
    public string? StabilizerHint
    {
        get => _stabilizerHint;
        init => _stabilizerHint = string.IsNullOrWhiteSpace(value) ? null : value;
    }

    /// <summary>
    /// 旧签名路径的兼容 shadow 别名。只允许用于 B2 divergence 对账，
    /// 不得解释为 identity；正式新输入应写 <see cref="StabilizerHint"/>。
    /// </summary>
