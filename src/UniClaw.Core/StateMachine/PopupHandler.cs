...
/// <summary>
/// UrgencyLevel enum (4 值) — LOW → CRITICAL。
/// </summary>
...
public sealed class PopupClassifier
{
    ...
    /// <summary>
    /// Dismiss strategy per popup type。
    /// </summary>
    public static readonly IReadOnlyDictionary<PopupType, DismissStrategy> DismissStrategyMap =
        new Dictionary<PopupType, DismissStrategy>
        {
            [PopupType.Permission] = DismissStrategy.AutoClose,
            [PopupType.Error] = DismissStrategy.Back,
            [PopupType.Ad] = DismissStrategy.WaitTimeout,
            [PopupType.Dialog] = DismissStrategy.AutoCloseOrBack,
            [PopupType.Unknown] = DismissStrategy.Back,
        };
    ...
    public PopupClassification Classify(string popupText, List<string>? availableButtons = null)
    {
        ...
        // 3. Determine dismiss strategy
        var dismissStrategy = DetermineDismissStrategy(popupType);
    ...
    private DismissStrategy DetermineDismissStrategy(PopupType popupType)
        => DismissStrategyMap.TryGetValue(popupType, out var strategy) ? strategy : DismissStrategy.Back;
...
public sealed class PopupActionExecutor
{
    ...
    private static PopupHandlingResult DefaultPermission(PopupContext ctx)
        => new PopupHandlingResult(true, "auto_close", "Clicked dismiss button for permission popup");

    private static PopupHandlingResult DefaultError(PopupContext ctx)
        => new PopupHandlingResult(true, "back", "Pressed back to dismiss error popup");

    private static PopupHandlingResult DefaultAd(PopupContext ctx)
        => new PopupHandlingResult(true, "wait_timeout", "Waited for ad popup to expire");

    private static PopupHandlingResult DefaultDialog(PopupContext ctx)
        => new PopupHandlingResult(true, "auto_close_or_back", "Auto-closed or backed out of dialog");

    private static PopupHandlingResult DefaultUnknown(PopupContext ctx)
        => new PopupHandlingResult(false, "back", "Pressed back for unknown popup");
}
...
public sealed class StateRestorer
{
    ...
    public string PreserveState(ITraversalContext context)
    {
        ...
        var preserved = new PreservedState(
            ...
            NodeStackDepth: context.NodeStack.Depth,
    ...
    /// <summary>
    /// 验证恢复后的状态完整性 — 失败则标记 handling result 为 failed。
    /// </summary>
    public StateValidationResult ValidateRestoredState(ITraversalContext context)
    {
...
public sealed class PopupHandler
{
    ...
    public PopupHandlingResult HandlePopup(string popupText, ITraversalContext context, List<string>? availableButtons = null)
    {
        ...
        // Step 6: validate
        var validation = _restorer.ValidateRestoredState(context);
