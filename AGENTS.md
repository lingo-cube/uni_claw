...
## 1.6 工作汇报规则（2026-09-23 所有者指令；对所有工作生效）

向人交付的一切汇报（Gate 汇报 / 评审处置 / 完成 / handoff）用
programmer-style handoff 格式，唯一真相源：`docs/agents/work-reporting.md`。
骨架：做了什么 → 原来的问题 → 这次修改 → 验证结果 → 新发现 → 下一步。
主线是代码与行为（调用链 / 输入输出 / 真实 diff / 修复前后行为对比）；
Gate / Acceptance / 评审编号（F/M/G/D/S…）只作括号引用，不做叙事主体。
仓库持久记录（state.md status log、spec 处置表、evidence）保持 ID 索引
形态不变。合格判据：读者看完能回答「打开 IDE 应该看哪几个文件、哪条
调用链、原来哪里错、现在怎么改、下一步准备碰哪里」。
