...
- Codex 默认读取 `AGENTS.md`；Claude Code 默认读取 `CLAUDE.md`，再由其中指针转到 `AGENTS.md`。
- 规则变更优先改本文件；Claude 专属 slash command / hook / workflow 规则仍放在 `.claude/`。

### Skills / Workflows 复用策略

Claude 项目内 skill 位于 `.claude/skills/`，workflow 位于 `.claude/workflows/`。当前先不为 Codex 复制或安装同名 skill：

- 可直接复用的知识型 skill：Codex 在任务匹配时可按需阅读对应 `.claude/skills/<name>/SKILL.md 或 skill.md` 作为项目 playbook。
- Claude 专属入口：`/skill`、`/Workflow`、`/opsx:*` 命令仍只在 Claude Code 环境中直接可用。
- 若后续确认某个 skill 需要被 Codex 自动发现，再将它迁移/包装成 Codex skill；迁移前保持 `.claude/skills/` 为单点来源，避免双份漂移。
...
| `openspec archive <change>` / `按 OpenSpec archive ...` | 完成归档、提取 decisions、同步主规格和四层文档 | `.claude/skills/openspec-archive-change/SKILL.md` |
