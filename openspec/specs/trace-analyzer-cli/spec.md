...
### Requirement: trace CLI exposes 6 subcommands

`uni-claw trace` SHALL expose exactly 6 subcommands:
...
### Requirement: exit codes follow a stable contract

Exit codes SHALL be: 0 = success (diagnosis completed); 1 = `diff` detected behavioral differences; 2 = usage error or run directory not found; 3 = empty trace (no spans).
