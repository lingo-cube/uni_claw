...
#### Scenario: determine_dismiss_strategy selects strategy based on target availability (D-10)

WHEN `find_dismiss_target()` has returned a result
THEN the third sub-method `determine_dismiss_strategy()` SHALL use conditional logic:
- IF a dismiss target was found → SHALL return `AutoClose` regardless of PopupType
- IF no dismiss target was found → SHALL fallback per PopupType:
  - PERMISSION → `WaitTimeout`
  - ERROR → `AutoCloseOrBack`
  - AD → `Back`
  - DIALOG / UNKNOWN → `Back` (default fallback)
...
#### Scenario: UrgencyLevel enum values (D-11: Critical removed)

WHEN `UrgencyLevel` is defined
THEN it SHALL contain exactly 3 values: LOW, MEDIUM, HIGH
AND the levels SHALL be ordered from least urgent (LOW) to most urgent (HIGH)
AND Critical SHALL NOT be present (removed as unreachable dead value, → D-11)
