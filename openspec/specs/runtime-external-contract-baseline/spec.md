...
with their exact current semantics. The Goal plane MAY grow only through additive
operations that preserve the frozen `run.start` operation; this change adds
`run.strategy.start` as a distinct start-time operation and does not realize the
deferred mid-Run Guidance plane.
