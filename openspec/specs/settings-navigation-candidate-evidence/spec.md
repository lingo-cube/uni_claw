# Settings Navigation Candidate Evidence Specification

## Purpose

Define the Settings-scoped, caller-independent evidence path that derives NAVIGATION_CANDIDATE / LOCAL_CONTROL / UNKNOWN interaction affordances from real Android UIAutomator hierarchy without requiring caller branch inventories or destination identity.

## Requirements

### Requirement: Structured Android UI evidence reaches Observation

The Runtime SHALL expose a narrow deterministic structured Android UI evidence source for accepted observed elements in Settings-scoped contexts. Evidence SHALL contain raw structured facts only, not semantic navigation claims, and SHALL NOT require destination page identity.
...
- **THEN** structured evidence independently makes that candidate discoverable without caller inventory supplying it

### Requirement: Runtime InteractionAffordanceEvidence

The Runtime SHALL provide an immutable InteractionAffordanceEvidence value with at least NAVIGATION_CANDIDATE, LOCAL_CONTROL, and UNKNOWN classifications. It SHALL NOT represent authorization, destination identity, branch completion, or Goal completion.
...
Implementation SHALL prove at least one real Settings evidence source before graduation. Synthetic-only evidence is forbidden.
