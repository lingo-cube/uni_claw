...
> BASELINE spec for the harness-side Assistance provider adapter (External
> Contract Plane 3 implementation). No code in this change. Seam:
> `runtime-assistance-seam` (implemented). Contract:
> `runtime-external-contract-baseline`.
