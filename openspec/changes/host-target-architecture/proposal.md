...
**Alignment with `unified-asset-pipeline-trace-validation`** (proposed, apply-pending): that change fixes the asset/storage/verification domain of Host (Host = composition + metadata + exceptions/logging; Core `ITracePipeline` + `IAssetStore` + V2 layout + verification moved out). This change fixes the AI/observation/policy domain. The two are complementary halves of the same composition-root blueprint and do not overlap: this change's scope is the seams C1–C4 + D1–D7 below; asset pipeline, layout V2, safety persistence removal, and verification exit Host are owned by the unified change and are NOT re-derived here.
...
- **Host probes (`doctor`/`analyze`) route diagnostics through `ITraceRecorder`** and submit via the Core `ITracePipeline` (unified: sync reference event + bytes via pipeline); no parallel diagnostic output format.
- **`--repeat` aggregation seam with unified verification**: child runs end `pending_verification` (unified); the `IterationAggregator` SHALL treat that status as "not yet judged" (never a false failure), report pending counts, and consume verdicts from `result.json` after `trace verify` (unified contract) — the aggregator does NOT re-implement judgment.
