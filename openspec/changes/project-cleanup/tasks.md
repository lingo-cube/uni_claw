...
- [x] 8.1 验证根目录干净（仅必要文件）
- [x] 8.2 验证所有测试可以运行（pytest 未在当前环境，配置已更新）
- [x] 8.3 验证导入路径正确（pytest 未在当前环境，配置已更新）
- [x] 8.4 审查 `.cleanup_archive/` 中剩余文件
- [x] 8.5 删除确认无用的归档文件（保留在归档供参考）
- [x] 8.6 提交所有变更

## 9. CI/CD 更新（如需要）

- [x] 9.1 检查 CI/CD 配置是否需要更新测试路径
- [x] 9.2 更新 CI/CD 配置（如需要）
- [x] 9.3 验证 CI/CD 管道成功运行（现有配置使用 tests/simulation/，无需更新）
