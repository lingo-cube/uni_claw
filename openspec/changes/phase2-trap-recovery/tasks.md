...
- [x] A1. **Trap model**: `Model/Trap.cs` — Trap(TrapKind, TrapScope, Source, Expected, Observed, Evidence, LastAction?)（HG-2 冻结: 7 字段，无 Recoverability）
...
- [x] B1. **Drift detection**（Agent）: Agent 在 post-action Observation 后检测 Agent-scope 漂移
...
- [x] B2. **Trap emission**（Agent）: 结构化 Trap 作为 Agent-scope escalate 载体
...
- [x] B3. **Agent Recovery path**: Trap(Scope=Agent) → Suspend Plan → Consume RecoveryAnchor.RestoreRecipe
...
- [x] B4. **Step-scope retry**（Traversal）: Select 失败 → Re-observe → Re-resolve → 成功 → Continue
      — 有界（max retry count）、确定性（同输入同重试序列）、仅 re-observe（不派发动作）
      — 重试耗尽 → Failed(Reason) → escalate to Agent（走 Phase 1 失败路径）
- [x] B5. **Recovery verification**: Verify 对照 VerificationCriteria — enriched failure reason with expectation vs actual
- [x] B6. **Phase 1 回归**: Phase 1 全部 5 Scenario 不受影响
...
- [x] C1. **ScriptedEnvironment 新 data variant**: `launcher-drift`（SC-P2-001）
...
- [x] C2. **ScriptedEnvironment 新 data variant**: `flicker-target`（SC-P2-002）
      — Network Settings 首次 Observe 只含 "Bluetooth" → 第二次 Observe 才含 "WiFi"
- [x] C3. **ScriptedEnvironment 新 data variant**: `unrecoverable`（SC-P2-003）
      — Relaunch(Settings) 后 ForegroundApplication 仍为 "Launcher"（恢复动作无效）
- [x] C4. **SC-P2-001 场景测试**（Scenario/AgentRecoveryLauncherDriftTests.cs）:
...
- [x] C5. **SC-P2-002 场景测试**（Scenario/StepRetryScenarioTests.cs）:
      flicker-target 变体 → Step retry → re-observe → re-resolve → Complete
      + 断言：journal 含重试条目、无 Trap 事件、Run 未 Failed、重试有界
- [x] C6. **SC-P2-003 场景测试**（Scenario/RecoveryVerificationFailureTests.cs）:
      unrecoverable 变体 → 恢复动作 → 验证失败 → Run Failed（Reason 显式）
      + 断言：Trace 含验证失败事件、无 Resume 标记、无恢复后的 Plan 步骤
- [x] C7. **Phase 1 全量回归**: 全部 104 测试 + 确定性重放 + 所有 Scenario
- [x] C8. **Full build + test**: `dotnet build` 0 警告 0 错误；`dotnet test` 全绿；`check-consistency.sh` ALL PASS

## Phase 2D — Guard Updates（Architecture Guards 文档化——Guard 5 修订已随 Phase 2A 原子完成，Guard 7 已随 Phase 2A closeout 预置）

- [x] D1. **Guard 7 receipt**: `docs/architecture/guards/guard-7-recovery-dependency-boundary.md` — Guard 7 围栏 Phase 2A 预置 (.gitkeep 平凡通过)，Phase 2B 首次非平凡执行，本文档为 receipt 记录
- [x] D2. Guard 1-4 + Guard 6 保持不削弱 — receipt 记录于 `docs/architecture/guards/` + `docs/decisions/phase2-architecture-receipt.md`
- [x] D3. 全部 Guards 通过（8/8）+ Phase 1 回归（158/158）

## Phase 2E — Final Verification（Phase Gate）

- [x] E1. Full build 0 警告 0 错误
- [x] E2. All tests: Phase 1 + Phase 2 → 158/158 全部通过
- [x] E3. Phase 2 Scenario assertions（SC-P2-001 / SC-P2-002 / SC-P2-003）全部满足
- [x] E4. Architecture Guards（1-4 + 5 revised + 6 + 7）8/8 全部通过
- [x] E5. `scripts/check-consistency.sh` ALL PASS
- [x] E6. Deterministic replay（SC-P1-001 断言 7 + SC-P2-001 恢复路径重放）全部通过
- [x] E7. Phase Boundary audit（Phase 3 deferred 保持缺席）PASS
- [x] E8. Phase 2 independent acceptance（runtime-validator）— CONDITIONAL_PASS (SPEC: stale docs reconciled, implementation correct)
