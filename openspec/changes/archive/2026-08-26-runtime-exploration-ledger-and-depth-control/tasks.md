> Reverification notice (2026-08-25): the later independent decision
> `docs/decisions/runtime-exploration-ledger-and-depth-control-graduation-reverification-decision.md`
> revoked graduation and unchecked the affected claims below. Earlier Evidence
> text is retained as history and must not be read as current completion proof.
...
### 8.3 Independent graduation reverification (2026-08-25, current HEAD)

- **GRADUATION REVOKED**. The earlier completion claims did not prove admission-
  derived/runtime-applied `ExplorationRule`, did not provide a strategy-declared
  bounded-record mode for arbitrary N, omitted structural-progress facts from
  ledger compilation, and double-counted real-path unresolved identities in
  `Discovered`.
- Affected tasks 2.2, 3.1, 3.3, 4.1, 4.2, 5.1, 6.1, 6.2, and 6.3 are reopened.
- Current targeted tests remain green (44/44), demonstrating a coverage gap
  rather than resolution. See the reverification decision for the complete
  Spec-to-evidence map and Human decision packet.
- Human selected Option A on 2026-08-25. The successor
  `runtime-exploration-semantic-admission-remediation` planning artifacts are
  complete and strictly valid.
- Historical status at the section-8.3 revocation point was
  `HUMAN_OPTION_A_SELECTED` / `HUMAN_APPLY_GATE_REQUIRED`; this line is retained
  as history and superseded by the authorized remediation and final evidence in
  section 8.4. Phase 3, archive, and new evidence ownership remain unauthorized.

### 8.4 Option A successor remediation and final independent evidence

- Human authorized the exact Option A successor Apply revision while excluding
  new wire/schema, Evidence owner, state system, scenario knowledge, Phase 3
  Memory, and Phase 4 dynamic depth. The applied successor is
  `runtime-exploration-semantic-admission-remediation`.
- The successor closes every finding from section 8.3: admission derives and
  carries one immutable D1 interpretation; the real Agent path consumes the
  closed rules before authorization/dispatch; the ledger is bound to accepted
  Run provenance and compiles an exact identity partition; structural facts are
  optional, validated correlation/digest evidence only; authority and wire
  boundaries remain unchanged.
- Leader independently inspected every production symbol and real-path test,
  rejected incomplete worker evidence repeatedly, and re-executed all final
  gates: build 0 warnings/0 errors; targeted Strategy/ledger/depth/unresolved/
  OpenWorld/GoalEvidence/FSM/Traversal/authority/neutrality 410/410 PASS; full
  deterministic Runtime excluding RealDevice, RealEmulator, RealityBaseline
  2052/2052 PASS; Semantic 32/32 PASS; both predecessor and successor strict
  OpenSpec validations PASS; workflow validation PASS; consistency C1-C12 ALL
  PASS; `git diff --check` PASS.
- Device-dependent paths were deliberately excluded from the deterministic
  graduation run and were not exercised in this session. No real-device or
  emulator capability conclusion is inferred from the deterministic result.
