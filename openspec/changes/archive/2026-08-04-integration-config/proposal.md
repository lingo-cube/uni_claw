...
集成测试的运行参数（provider/model/outputRoot/scope 门控/视觉服务 env）散落在测试代码硬编码与手动 export 中：漏设 `UNICLAW_INTEGRATION_PROVIDER` 即静默撞云端 Sensenova 空响应（实测 3m33s），outputRoot 命名不可预测，CI/本地差异靠记忆。运行配置需要**单点真源 + fail-fast 校验 + env 覆盖通道**。阶段 2 主体已实施（D-202–D-207，9 个配置单测 + 6 个预检单测通过）；本 change 将其正式化并收敛剩余配置边界问题（P2.5/P2.6/P2.8/P2.9）。

## What Changes

- **新增 `tests/UniClaw.Host.Tests/Integration/integration.config.json`**（schema `uniclaw.integrationConfig.v1`）——L1 测试运行配置单点真源：`emulator`（serial/outputRoot/runNaming/keepRuns/recordBaseline）、`providers`（按 id 分块；`visionServer` 只挂 local）、`scenarios`（绑定 provider/mode/timeout）
...
- **新增文档**——`docs/testing/integration-config.md`（配置规范：7 层职责边界/§1-§11，含 D-202–D-207 决策）、`docs/testing/integration-pipeline-issues.md`（问题台账，P1–P6 域状态追踪）
- **台账边界项**（integration-pipeline-issues.md 追踪，决策或实施不在本 change）：
  - P2.5：`recordBaseline`/`keepRuns` 配置键已落地（定义 + 校验），**无消费方** —— 消费逻辑属阶段 5（基线录制/清理，独立 change）
  - P2.6：deepseek 是 Host 真实 provider（内部路由 id：qwen two_stage 模式 [HostCommands.cs:1231](src/UniClaw.Host/Commands/HostCommands.cs#L1231)、local text 路由 [:1271](src/UniClaw.Host/Commands/HostCommands.cs#L1271)、UniBrain 默认 [:1125](src/UniClaw.Host/Commands/HostCommands.cs#L1125)）但不在 config `KnownProviders` —— ✅ **D-208 已定案**：内部 text 路由键，不进 config（声明已落 integration-config.md §3），本 change 仅验证对齐
  - P2.8：`UNICLAW_VISION_MODE` 一变量两义（视觉模式 [HostCommands.cs:1216](src/UniClaw.Host/Commands/HostCommands.cs#L1216) vs run mode [:1605](src/UniClaw.Host/Commands/HostCommands.cs#L1605)）—— ✅ **D-209 已定案**：拆分 `UNICLAW_RUN_MODE`，读点取证完成；实施属 **Host 生产代码变更**，台账跟踪、独立 change，不在本 change
  ...
  - P1.1：locate 最终校验身份恒空（`CurrentPath=[]`）——验证域问题，台账跟踪、独立 change，本 change 不涉及（验收 6.2 依赖其解决）
