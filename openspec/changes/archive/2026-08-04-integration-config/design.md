...
集成测试链路的运行参数此前散落三处：测试代码硬编码（locate→sensenova、enumerate→mock）、手动 export（视觉服务 6 个 env）、无配置真源（scope 门控只走 env）。漏设 provider env 即静默撞云端空响应（实测 3m33s）。阶段 2 主体已在 `feature/refactor` 分支实施（**未经 OpenSpec change 追踪**），本设计将其正式化，并给出剩余边界问题的决策建议。
...
**已确认缺口**：P2.5（`recordBaseline`/`keepRuns` 只定义+校验、无消费方）；P2.8（`UNICLAW_VISION_MODE` 双读点：[:1216](src/UniClaw.Host/Commands/HostCommands.cs#L1216) 视觉模式 single/two_stage、[:1605](src/UniClaw.Host/Commands/HostCommands.cs#L1605) run mode mode-a，`UNICLAW_RUN_MODE` 尚不存在）。P2.6（deepseek 不在 `KnownProviders`）已由台账 **D-208 定案**（内部 text 路由键，不进 config）；P2.8 已由台账 **D-209 定案**（拆分方向，实施待办）——详见下文"台账决策引用"。验证域问题（P1.1 最终校验身份恒空 等）由台账跟踪，不在本 change。
...
- 决策对齐：D-202–D-207 入档；P2.6（D-208）/P2.8（D-209）与台账决策注册一致，D-210（装配层归属）本 change 记录

**Non-Goals:**

- **不改 `src/` 生产代码**——config 是测试侧装配层，经 env 通道注入；Host 的 CLI env 回退（L3 内侧）保持为直跑兜底
- P2.8 拆分 `UNICLAW_VISION_MODE`（Host 生产代码变更；D-209 已定案，实施在台账跟踪，不在本 change 实施）
- P2.9 统一命名前缀（推迟，边界已文档化）
- P2.5 `recordBaseline`/`keepRuns` 消费逻辑（阶段 5 基线/清理，独立 change；本 change 只保证配置键就绪）
- 验证域问题（P1.1 最终校验身份恒空 等）——台账跟踪、独立 change，不在本 change（验收 6.2 依赖其解决）
...
- **D-208 | deepseek = 内部 text 路由键，不进 config providers（P2.6）** — 证据：`CreateProviders` 无 `providerId=="deepseek"` 分支（落到 claude 检查抛 "does not expose vision capability"，用户无法 `--provider deepseek`）；deepseek 键由 local（独立 DEEPSEEK_* 凭据）与 qwen two_stage（复用 qwen 凭据）各自装配"文本推理角色"。声明已落 integration-config.md §3 providers 表。本 change 无实施项。
...
- **P2.8 一变量两义是现存风险**（设 `UNICLAW_VISION_MODE=single` 会把 run mode 设成非法值）→ 本 change 不实施修复；design 已记录读点语义（1216 视觉 / 1605 run mode），台账 P2.8 保持 ⏳ 待用户决策
