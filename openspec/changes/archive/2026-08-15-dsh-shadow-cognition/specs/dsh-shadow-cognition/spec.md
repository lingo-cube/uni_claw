# Spec: dsh-shadow-cognition

> Design-only baseline for the minimum DSH-native Shadow Cognition capability.
> Pinned DSH baseline: `47f943859bef60e4160492346772ded9b24f765a`
> (`0.1.0-rc.5`), read-only. Frozen prerequisites: `dsh-uniclaw-control-plane-protocol-baseline`
> and `dsh-uniclaw-control-plane-plugin-implementation` (both archived,
> graduated). This change defines WHAT the Shadow baseline SHALL be; design.md
> and source-evidence-matrix.md define HOW and the DSH-source evidence.

> **REBASELINE 2026-08-15** — `PROJECT_LEADER_DSH_SHADOW_COGNITION_DURABILITY_REBASELINE_DECISION`
> (`BASELINE_ASSUMPTION_FALSIFIED`). V1 durability is rebaselined from
> `DSH_NATIVE_DURABLE` (durable `shadow/analysis` session event with
> `ignorable: true`) to `EPHEMERAL_PROCESS_LOCAL`: **Shadow V1 appends ZERO
> custom session events** (no `shadow/analysis` event exists; no `ignorable`
> marker is needed because nothing is appended). V1 triggers are frozen to
> `human.request` ONLY; `run.failed`/`run.completed` auto triggers are DEFERRED
> (`AutoTriggersDeferredUntilConsumerExists = YES`). The original buyer
> (post-hoc / near-live run interpretation) remains satisfied: a human request
> can deterministically reconstruct fresh context from the Kernel read surface
> and recompute analysis after a DSH restart.
