# Spec: open-world-traversal-identity-safety

## ADDED Requirements

### Requirement: Agent-owned run-local traversal identity evidence

The Runtime SHALL maintain Agent-owned, run-local identity evidence during open-world traversal, consisting of at least:
...
This evidence SHALL be local to the open-world traversal path and SHALL NOT be exposed as a global graph, persistent route model, or new state owner.
...
Before creating a child Container from fresh reconciled evidence in the open-world path, if the child semantic page identity already exists in the current ancestry set, the Runtime SHALL reject the transition as a cycle and SHALL NOT create the child Container, dispatch another action, or claim progress.
