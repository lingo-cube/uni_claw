...
> Spec-driven definition of the authorized async run-start entry slice.
> Buyer: DSH outer agent needs an authorized cross-process entry to start an
> existing `Runtime.Agent` semantic run as a deterministic execution subagent.
> Source baseline verified 2026-08-17 (uni-agent branch).
