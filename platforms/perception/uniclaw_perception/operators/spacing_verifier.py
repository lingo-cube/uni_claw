...
**Structural title-column exemption (WI-PFW-S2fix2).**  The C4 column-spread
check as described is a *uniform-list* shape assumption (all rows on one
column).  Real relation-head child pages can carry one structural shape the
uniform-list model never produces: a page-TITLE band on the far-left column
with the menu content rows on a single indented dominant column
(e.g. x1 69/213/214/208).  For exactly that shape the topmost band is exempted
from the C4 spread computation — it is a title column, not a misaligned menu
row — and C4 is re-computed over the remaining (dominant-column) bands only.
The exemption is structural and fail-closed: it fires ONLY when (a) all
non-topmost bands form ONE modal column cluster with ≥2 members (within the
verifier's per-side column tolerance), and (b) the topmost band lies LEFT of
that cluster by more than the tolerance.  Every other mixed-column shape keeps
the original full-set spread check (veto).  No threshold is relaxed: the same
``columnToleranceFloor``/``columnToleranceRatio``/``minStepRatio`` bounds and
the full-band vertical-cadence check remain exactly as declared.
...
def _geometry_violation(
    ...
    The C4 column-spread check runs over the non-exempt bands: when the page
    shape is exactly a structural title column (the TOPMOST band to the LEFT
    of one dominant content column formed by every other generated band),
    the topmost band is exempted and the spread is re-computed over the
    dominant-column bands only.  Every other mixed-column shape keeps the
    original full-set check (fail-closed); no tolerance is relaxed.
    """
    ...
    # C4: same-column alignment over the non-exempt bands.
    column_bound = max(
        p.column_tolerance_floor, 2.0 * p.column_tolerance_ratio * median_gap
    )
    exemption = _title_column_exemption(ordered, p, median_gap)
    spread_bands = generated
    if exemption is not None:
    ...
    if spread > column_bound:
        scope = "non-exempt generated rows" if exemption is not None else "generated rows"
        return (
    ...
    # median step (computed over ALL generated rows; the exemption only
    # re-scopes the C4 spread, never the cadence bound).
    if median_gap > 0:
        min_gap = min(gaps)
