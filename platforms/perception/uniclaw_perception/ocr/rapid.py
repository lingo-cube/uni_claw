...
def configure_ocr_models(*, language: str | None = None,
    ...
    """Resolve rec-model kwargs directly from package config（PER-007 R1）.

    Replaces the legacy governance ``ocr_model_manifest`` resolution: the
    Target tree carries exactly one managed rec artifact set (en PP-OCRv4
    mobile rec + dictionary), addressed by ``config.ocr_rec_model`` /
    ``config.ocr_dict`` (env ``UNICLAW_OCR_MODEL`` / ``UNICLAW_OCR_DICT``
    override; package-relative defaults).  Resolution semantics preserved:
      * ``en`` (default) → managed model + dict; either missing on disk →
        **fail closed** (raise; never silently fall back to the default
        Chinese model)
      * ``zh`` → ``{}`` (rapidocr package-default weights; pre-registration
        behavior preserved)
      * any other language → fail closed (unmanaged language)
    ...
    cfg = get_config()
    lang = (language or cfg.ocr_lang or "en").lower()
    if lang == "zh":
        return {}
    if lang != "en":
        raise RuntimeError(
            f"no managed OCR rec model for language={lang!r}; "
            "refusing to silently fall back to the default model")
    model_path = Path(cfg.ocr_rec_model)
    if not model_path.exists():
        raise RuntimeError(
            f"managed OCR rec model missing on disk: {model_path}")
    dict_path = Path(cfg.ocr_dict)
    if not dict_path.exists():
        raise RuntimeError(
            f"managed OCR dictionary missing on disk: {dict_path}")
    return {
